...
// the runtime auto-subscribes to a Server-Sent Events live-reload stream and
// reloads the page on file changes. It also posts size updates to the parent
// window so Publisher.embed() in the host can resize the iframe.
...
(function () {
   ...
   var ctx = pathMatch
      ? { environment: pathMatch[1], package: pathMatch[2] }
      : {};
   ...
   function setUpEmbeddedSelfBehaviors() {
      ...
      if (inIframe) {
         var lastHeight = -1;
         function postSize() {
            // documentElement.scrollHeight is the most reliable measure of
            // "height needed to display the full page without scrolling."
            var h = Math.max(
